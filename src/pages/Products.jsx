...
const Products = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      listProducts(); // Refresh danh sách sau khi xoá
    } catch (error) {
              ...
              <table className="w-full">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Giá tiền</th>
                    ...
                    <tr key={product.id} className="hover:bg-gray-50 transition-colors duration-150">
                      ...
                      <td className="px-6 py-2 whitespace-nowrap">
                        <div className="w-14 h-14 bg-gray-100 rounded-lg overflow-hidden border border-gray-200">
                          {Object.entries(product.listImageProduct).map(([key, value]) => (
                            <img key={key} src={value.url} alt={product.name} className="w-full h-full object-contain" />
                          ))}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{product.createAt}</td>
